...
describe("generateScenarioWithRepair: photo_clip 注入", () => {
  const clip = {
    type: "photo_clip",
    position: "center",
  ...
  const inject = { role: "error_location", spec: clip } as const;

  it("AI 経路: error_location に visualAid が無ければ注入する", async () => {
    const result = await generateScenarioWithRepair({ jobId: "job-6", approved, run: async () => cleanOutput(), inject });
    const panel = result.plan.panels.find((item) => item.role === "error_location");
    expect(panel?.visualAid).toEqual(clip);
    expect(mangaPlanV3Schema.safeParse(result.plan).success).toBe(true);
    // 他のコマには入れない
    expect(result.plan.panels.filter((item) => item.visualAid?.type === "photo_clip")).toHaveLength(1);
  });

  it("AI が error_location に図解を付けていたら上書きしない", async () => {
    const parsed = JSON.parse(cleanOutput());
    parsed.panels[1].visualIntent = { type: "equal_groups", requirement: "required", total: { numerator: 24, denominator: 1 }, groupCount: 3 };
    const result = await generateScenarioWithRepair({ jobId: "job-7", approved, run: async () => JSON.stringify(parsed), inject });
    const panel = result.plan.panels.find((item) => item.role === "error_location");
    expect(panel?.visualAid?.type).toBe("bar_model");
  });

  it("フォールバック経路でも注入される", async () => {
    const result = await generateScenarioWithRepair({
      jobId: "job-8", approved,
      run: async () => { throw new Error("codex down"); },
      maxAiAttempts: 1,
      inject
    });
    expect(result.usedFallback).toBe(true);
