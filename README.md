...
公開素材は `apps/web/public/assets/`、カタログは `packages/renderer/assets/metadata.json` に置きます。素材選択はキャラクター、感情、行動、story beat、向き、配置をスコアリングし、同点ではassetId順に決定します。素材原本・生成候補・宿題写真はGitHubへ保存しません。

GitHub Pagesへのデプロイはルートの `.github/workflows/homework-manga-pages.yml` を使用します。Firebase Web設定はGitHub Actions secretsへ登録してください。


## Web upload via dedicated GAS

The root page accepts a homework photo after Google login. The browser compresses the image, and the dedicated GAS deployment stores it in Google Drive. Slack receives only the `[homework-web] <jobId>` processing trigger; Firebase supplies progress and results to the web UI. Jobs submitted while the local worker is stopped are recovered at its next startup.

Configure these GAS Script Properties: `FIREBASE_DATABASE_URL`, `FIREBASE_WEB_API_KEY`, `HOMEWORK_OWNER_UID`, `HOMEWORK_DRIVE_FOLDER_ID`, `SLACK_BOT_TOKEN`, and `SLACK_HOMEWORK_CHANNEL_ID`. Deploy GAS as a web app, then register its URL as the GitHub Actions secret `VITE_HOMEWORK_GAS_API_URL`.
