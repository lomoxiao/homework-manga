...
function chart(type:string,d:Record<string,unknown>){const values=(Array.isArray(d.values)?d.values:String(d.values??"").split(",")).map(Number).filter(Number.isFinite);if(!values.length)return tableSvg("グラフ",d);const max=Math.max(...values,1);const points=values.map((v,i)=>`${30+i*330/Math.max(1,values.length-1)},${95-v/max*70}`).join(" ");const bars=values.map((v,i)=>`<rect x="${25+i*350/values.length}" y="${95-v/max*70}" width="${300/values.length}" height="${v/max*70}"/>`).join("");return `<svg viewBox="0 0 400 110" role="img" aria-label="グラフ"><g fill="#80bfff" stroke="#334">${type==='line_chart'?`<polyline points="${points}" fill="none" stroke-width="3"/>`:bars}</g></svg>`}
