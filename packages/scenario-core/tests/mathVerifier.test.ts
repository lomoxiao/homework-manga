import { verifyDraft, verifyEquation } from "../src/mathVerifier";
...
describe("deterministic math verification", () => {
  ...
  it("fails closed for construction", () => {
    expect(verifyDraft({ ...defaultDraft, problemType: "construction", canonicalAnswer: "", correctAnswer: "作図" }).status).toBe("unsupported");
  });
});
